...
function Client(name, root, Client_start_var, Client__d_var, Client__b_var, Client__c_var, Client__a_var, Client_counter_var, Client__e_var, debug) {
	this.name = name;
...
Client.prototype.build = function(session) {
	...
	Client_null_RUN.to(Client_null_STOP).when((m3) => {
		return m3._port === 'app' && m3._msg === 'm3' && (this.Client_counter_var >= 5);
	});
	Client_null_RUN.to(Client_null_RUN).when((m3) => {
		return m3._port === 'app' && m3._msg === 'm3' && (m3.a === this.Client__a_var && this.Client_counter_var < 5);
	}).effect((m3) => {
