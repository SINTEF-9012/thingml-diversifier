...
function Client(name, root, Client__e_var, Client__d_var, Client_counter_var, Client__a_var, Client__b_var, Client__c_var, debug) {
	this.name = name;
...
Client.prototype.build = function(session) {
	...
	let Client_null_RUN = new StateJS.State('RUN', this._statemachine).entry(() => {
		...
		process.stdout.write(''+', ');
		process.stderr.write('\n');
		setImmediate(() => this.bus.emit('app?m1bis', this.Client__a_var, this.Client__b_var, this.Client__c_var, this.Client__d_var, this.Client__e_var));
		...
		process.stdout.write(''+', ');
		process.stderr.write('\n');
		setImmediate(() => this.bus.emit('app?m2bis', this.Client__a_var, this.Client__b_var, this.Client__c_var));
	...
	Client_null_RUN.to(Client_null_RUN).when((m3bis) => {
		return m3bis._port === 'app' && m3bis._msg === 'm3bis' && (m3bis.a === this.Client__a_var);
	}).effect((m3bis) => {
...
Client.prototype.receivem3bisOnapp = function(a) {
	this._receive({_port:"app", _msg:"m3bis", a:a});
}
