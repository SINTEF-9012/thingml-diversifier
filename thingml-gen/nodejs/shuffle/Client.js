...
function Client(name, root, Client__b_var, Client__c_var, Client__d_var, Client__a_var, Client__e_var, Client_counter_var, debug) {
	this.name = name;
...
Client.prototype.build = function(session) {
	...
	let Client_null_RUN = new StateJS.State('RUN', this._statemachine).entry(() => {
		...
		process.stdout.write(''+', ');
		process.stderr.write('\n');
		setImmediate(() => this.bus.emit('app?m1', this.Client__e_var, this.Client__d_var, this.Client__c_var, this.Client__a_var, this.Client__b_var));
		...
		process.stdout.write(''+', ');
		process.stderr.write('\n');
		setImmediate(() => this.bus.emit('app?m2', this.Client__c_var, this.Client__b_var, this.rnd(), this.Client__a_var, this.rnd()));
...
Client.prototype.receivem3Onapp = function(a, r3, r5, r2, r4) {
	this._receive({_port:"app", _msg:"m3", a:a, r3:r3, r5:r5, r2:r2, r4:r4});
}
