...
function Client(name, root, Client__b_var, Client__d_var, Client_counter_var, Client__c_var, Client__e_var, Client__a_var, debug) {
	this.name = name;
...
Client.prototype.build = function(session) {
	...
	let Client_null_RUN = new StateJS.State('RUN', this._statemachine).entry(() => {
		...
		process.stdout.write(''+', ');
		process.stderr.write('\n');
		setImmediate(() => this.bus.emit('app?m2', this.rnd(), this.Client__a_var, this.Client__b_var, this.rnd(), this.Client__c_var));
...
Client.prototype.receivem3Onapp = function(r2, r4, r3, r5, a) {
	this._receive({_port:"app", _msg:"m3", r2:r2, r4:r4, r3:r3, r5:r5, a:a});
}
