...
App.prototype.build = function(session) {
	...
	let App_null_SendAck = new StateJS.State('SendAck', this._statemachine).entry(() => {
		...
		process.stdout.write(''+', ');
		process.stderr.write('\n');
		setImmediate(() => this.bus.emit('app?m3', this.rnd(), this.rnd(), this.rnd(), this.rnd(), this.App_id_var));
	});
...
App.prototype.receivem2Onapp = function(r1, a, b, r0, c) {
	this._receive({_port:"app", _msg:"m2", r1:r1, a:a, b:b, r0:r0, c:c});
}
