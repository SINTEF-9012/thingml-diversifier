...
#include <Arduino.h>
int minFree = 0x7FFF;
__attribute__((noinline)) void checkFreeRam() {
...
void App_handle_app_m1(struct App_Instance *_instance, uint8_t a, uint8_t b, int16_t c, uint8_t d, uint8_t e);
...
#define APP_NULL_WAITFORM1_STATE 1
#define APP_STATE 2
#define APP_NULL_SENDACK_STATE 3
...
int16_t Client_counter_var;
uint8_t Client__c_var;
uint8_t Client__e_var;
int16_t Client_start_var;
...
// Declaration of callbacks for incoming messages:
void register_Client_send_app_m1_listener(void (*_listener)(struct Client_Instance *, uint8_t, uint8_t, int16_t, uint8_t, uint8_t));
void register_external_Client_send_app_m1_listener(void (*_listener)(struct Client_Instance *, uint8_t, uint8_t, int16_t, uint8_t, uint8_t));
...
Serial.print(")!");
Serial.print("\n");
;uint8_t m3Arg0 = _instance->App_id_var;
Serial.print("!");
Serial.print("0");
Serial.print(',');
Serial.print(((m3Arg0 >> 0) & 0xFF));
Serial.print(',');
Serial.print("\n");
Serial.print(":");
Serial.print("Byte");
...
Serial.print(',');
Serial.print("\n");
App_send_app_m3(_instance, m3Arg0);
break;
}
default: break;
}
...
//Prototypes: Message Sending
void Client_send_app_m1(struct Client_Instance *_instance, uint8_t a, uint8_t b, int16_t c, uint8_t d, uint8_t e);
...
_instance->Client__c_var = f_Client_rnd(_instance) % 20;
_instance->Client__d_var = f_Client_rnd(_instance) % 10 + 20;
_instance->Client__e_var = f_Client_rnd(_instance) % 5 + 10;
...
Serial.print(")!");
Serial.print("\n");
;uint8_t m1Arg0 = _instance->Client__a_var;
;uint8_t m1Arg1 = _instance->Client__b_var;
;int16_t m1Arg2 = _instance->Client__c_var;
;uint8_t m1Arg3 = _instance->Client__d_var;
;uint8_t m1Arg4 = _instance->Client__e_var;
Serial.print("!");
Serial.print("0");
Serial.print(',');
Serial.print(((m1Arg0 >> 0) & 0xFF));
Serial.print(',');
Serial.print(((m1Arg1 >> 0) & 0xFF));
Serial.print(',');
Serial.print(((m1Arg2 >> 8) & 0xFF));
Serial.print(',');
Serial.print(((m1Arg2 >> 0) & 0xFF));
Serial.print(',');
Serial.print(((m1Arg3 >> 0) & 0xFF));
Serial.print(',');
Serial.print(((m1Arg4 >> 0) & 0xFF));
Serial.print(',');
Serial.print("\n");
Serial.print(":");
Serial.print("Byte");
...
Serial.print(',');
Serial.print("\n");
Client_send_app_m1(_instance, m1Arg0, m1Arg1, m1Arg2, m1Arg3, m1Arg4);
...
Serial.print(")!");
Serial.print("\n");
;uint8_t m2Arg0 = _instance->Client__a_var;
;uint8_t m2Arg1 = _instance->Client__b_var;
;uint8_t m2Arg2 = _instance->Client__c_var;
Serial.print("!");
Serial.print("0");
Serial.print(',');
Serial.print(((m2Arg0 >> 0) & 0xFF));
Serial.print(',');
Serial.print(((m2Arg1 >> 0) & 0xFF));
Serial.print(',');
Serial.print(((m2Arg2 >> 0) & 0xFF));
Serial.print(',');
Serial.print("\n");
Serial.print(":");
Serial.print("Byte");
...
Serial.print(',');
Serial.print("\n");
Client_send_app_m2(_instance, m2Arg0, m2Arg1, m2Arg2);
...
Serial.print("\n");
;int16_t duration = millis() - _instance->Client_start_var;
Serial.print("#CLI: took ");
Serial.print(duration);
Serial.print("ms.");
Serial.print("\n");
Serial.print("#CLI: used max ");
Serial.print(2048-minFree);
Serial.print("bytes.");
Serial.print("\n");
_instance->active = false;
break;
...
if (Client_State_event_consumed == 0 && _instance->Client_counter_var >= 100) {
...
else if (Client_State_event_consumed == 0 && a == _instance->Client__a_var && _instance->Client_counter_var < 100) {
Client_OnExit(CLIENT_NULL_RUN_STATE, _instance);
_instance->Client_State = CLIENT_NULL_RUN_STATE;
...
// Observers for outgoing messages:
void (*external_Client_send_app_m1_listener)(struct Client_Instance *, uint8_t, uint8_t, int16_t, uint8_t, uint8_t)= 0x0;
void (*Client_send_app_m1_listener)(struct Client_Instance *, uint8_t, uint8_t, int16_t, uint8_t, uint8_t)= 0x0;
void register_external_Client_send_app_m1_listener(void (*_listener)(struct Client_Instance *, uint8_t, uint8_t, int16_t, uint8_t, uint8_t)){
  checkFreeRam();
...
void enqueue_Client_send_app_m1(struct Client_Instance *_instance, uint8_t a, uint8_t b, int16_t c, uint8_t d, uint8_t e){
  checkFreeRam();
...
u_c.p = c;
_fifo_enqueue(u_c.bytebuffer[0] & 0xFF );
_fifo_enqueue(u_c.bytebuffer[1] & 0xFF );
...
//New dispatcher for messages
void dispatch_m1(uint16_t sender, uint8_t param_a, uint8_t param_b, int16_t param_c, uint8_t param_d, uint8_t param_e) {
  checkFreeRam();
if (sender == app_var.id_diversified) {
...
u_m1_c.bytebuffer[1] = mbuf[mbufi_m1 + 1];
mbufi_m1 += 2;
union u_m1_d_t {
uint8_t p;
byte bytebuffer[1];
