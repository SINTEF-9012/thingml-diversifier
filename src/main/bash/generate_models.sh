...
for LANGUAGE in ${LANGUAGES[@]}; do
    ...
    java -jar $DIVERSIFIER $BASEMODEL 1 onlysummary $MODELSDIR/$LANGUAGE 1

    echo "-- GENERATING STATIC DIVERSIFICATED MODELS --"
    java -jar $DIVERSIFIER $BASEMODEL $N defaultsummary $MODELSDIR/$LANGUAGE/static 1

    echo "-- GENERATING DYNAMIC DIVERSIFICATED MODELS --"
    java -jar $DIVERSIFIER $BASEMODEL $N runtimesummary $MODELSDIR/$LANGUAGE/dynamic 1
done
